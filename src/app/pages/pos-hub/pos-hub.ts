...
export class PosHub implements OnInit, OnDestroy {
  ...
  async enter(b:any){await this.run(async()=>{this.bid=b.id;this.tab='dashboard';this.resetCart();this.report=null;this.team=null;await this.refresh();if(this.rights.role==='kitchen')this.tab='kitchen';});}
  async refresh(){if(!this.bid)return;const bid=this.bid;const data=await this.request('/business/'+bid+'/state');if(bid!==this.bid)return;this.data=data;if(this.tab!=='admin'&&!this.visibleTabs.some(t=>t[0]===this.tab))this.tab='dashboard';this.cd.detectChanges();}
  ...
  async navigate(tab:string){this.tab=tab;this.menuOpen=false;if(tab==='team')await this.loadTeam();if(tab==='audit')await this.loadAudit();if(tab==='reports')await this.loadReport();if(tab==='admin')await this.loadAdmin();}
  ...
  async saveStaff(){await this.run(async()=>{await this.request(this.path('/team'),'POST',this.staff);this.staff={name:'',email:'',password:'',role:'waiter',can_pay:true};this.team=await this.request(this.path('/team'));this.notice='Empleado creado con acceso a este local.';});}
